function Result = SignificanceTesting_EXP(Data,PerformInvFormale,PerformCSN)
%% Inputs description:
% Data: Is a 1xN array contains N observations (hypothetically exponentially distributed)
...
for Xmin_i = 1:size(XminA,2)-1
    ...
    Avg_X = mean(Sample);
    %%% Estimate Beta
    Beta = 1./(Avg_X-Xmin); XMax = max(Sample);
    BetaFunction =@(B) abs(B.*(Avg_X-Xmin)+(B.*(XMax-Avg_X)+1).*exp(-B.*(XMax-Xmin))-1);
    Beta = lsqnonlin(BetaFunction,Beta,0,inf,options);
    %%% Probability Integral Transform
    CDF_Th = linspace(0,1,NumFittedSample);
    Delta = exp(-Beta.*(XMax-Xmin));
    CDF_Empirical = 1-exp(-Beta*(Sample-Xmin));
    ...
    Sample_Beta = (PreDist<=KSDistance)*Sample_Beta + (PreDist>KSDistance)*Beta;
...
Result.Data = Data;
Result.Sample_Beta = Sample_Beta;
...
clearvars -except Data PerformInvFormale PerformCSN XminANumPartition CSN_NSim Result...
            Sample_Beta Sample_Xmin Sample_KSDistance Sample_DistributionNoise Sample_NumFittedSample 
...
if PerformCSN==1
    ...
    for Run_i = 1:CSN_NSim
        ...
        ResampledSample  = Sample_Xmin -log(1-rand(1,SampleSize))/Sample_Beta;
